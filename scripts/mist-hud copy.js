...
export class MistHUD extends Application {
  ...
  getData(options) {
    ...
    // Crew themes and active crew theme
    data.crewThemes = this.getCrewThemes();
    data.crewTheme = this.getActiveCrewTheme();

    // Additional debug logging
    console.log("Accessible Crew Themes:", data.crewThemes);
    console.log("Active Crew Theme:", data.crewTheme);
  ...
  getCrewThemes() {
    ...
    console.log("Owned Crews:", ownedCrews); // Debug log
  ...
  getActiveCrewTheme() {
    ...
    // Retrieve all accessible crew themes
    const crewThemes = this.getCrewThemes();
    console.log("Accessible Crew Themes:", crewThemes);

    if (!crewThemes.length) {
        console.warn("No accessible crew themes found.");
        return undefined;
    }

    // Select the first crew theme by default
    const activeTheme = crewThemes[0];
    console.log("Selected Active Crew Theme:", activeTheme);

    // Locate the crew actor using the actorId included in crewThemes
    const crewActor = game.actors.get(activeTheme.actorId);
    console.log("Crew Actor:", crewActor); // Add this log

    if (!crewActor) {
        console.warn(`Crew actor for theme "${activeTheme.name}" not found.`);
        return activeTheme; // Return partial data if the crew actor is missing
    }

    // Retrieve and combine full theme data
    const crewThemeData = this.getCrewThemeData(crewActor, activeTheme.id);
  ...
  getCrewThemeData(crewActor, themeId) {
    if (!crewActor) {
        console.error("Invalid crewActor passed to getCrewThemeData. Ensure actor exists.");
        return {};
    }

    const themeItem = crewActor.items.find(item => item.type === "theme" && item.id === themeId);
    if (!themeItem) {
        console.warn(`Theme with ID "${themeId}" not found on crew actor "${crewActor.name}".`);
        return {};
    }

    // Retrieve and process tags
    const tagItems = crewActor.items.filter(item => item.type === "tag");
    ...
    const powerTags = this.getPowerTags(themeId, tagItems, subTagsByParent, crewActor);
    const weaknessTags = this.getWeaknessTags(themeId, tagItems, subTagsByParent, crewActor);

    // Retrieve mystery with prefix and icon
    // const mystery = this.getMysteryFromTheme(crewActor, themeId.id || themeId); // Ensure only the ID is passed

    // Retrieve improvements for the theme
    const improvements = crewActor.items
        .filter(item => item.type === "improvement" && item.system.theme_id === themeId)
        .map(improvement => ({
            id: improvement.id,
            name: improvement.name,
            description: improvement.system.description || "",
            uses: improvement.system.uses || { max: 0, current: 0, expended: false },
        }));
  ...
  getPowerTags(themeId, tagItems, subTagsByParent, crewActor) {
    const powerTags = tagItems.filter(tag => tag.system.theme_id === themeId && tag.system.subtype === "power");

    console.log("Retrieved Power Tags:", powerTags.map(tag => ({
        id: tag.id,
        name: tag.name,
        theme_id: tag.system.theme_id,
    })));

    return this.formatTagsHierarchy(powerTags, subTagsByParent).map(tag => {
        return this.applyBurnState(crewActor, tag.id); // Pass the crewActor here
    });
  }

  getWeaknessTags(themeId, tagItems, subTagsByParent, crewActor) {
      const weaknessTags = tagItems.filter(tag => tag.system.theme_id === themeId && tag.system.subtype === "weakness");

      console.log("Retrieved Weakness Tags:", weaknessTags.map(tag => ({
          id: tag.id,
          name: tag.name,
          theme_id: tag.system.theme_id,
      })));
      ...
      return this.formatTagsHierarchy(weaknessTags, subTagsByParent).map(tag => {
          return this.applyBurnState(crewActor, tag.id, "weakness"); // Pass the crewActor here
      });
  }
  ...
  applyBurnState(actor, tagId, tagType = "power") {
    const tagItem = actor.items.get(tagId);
    if (!tagItem || !tagItem.system) {
        console.warn(`Invalid or missing tag item for ID: ${tagId}. Ensure the tag exists in the actor's items.`);
    ...
    console.log("Tag Item Data:", tagItem);
